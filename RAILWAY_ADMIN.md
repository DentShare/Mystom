...
Логи веб-админки и бота пишутся в **Deploy Logs** (вкладка **Deploy Logs** у сервиса, не Build Logs). При открытии админки из бота и при нажатии «Обновить список» ищите строки с префиксом **`[admin_webapp]`**, например:
- `[admin_webapp] GET / — открыта главная, Host=...` — загрузка страницы;
- `[admin_webapp] api_users — запрос с Host=...` — запрос списка пользователей.

Если таких строк нет — откройте админку именно из кнопки бота («Админка (Web App)») и обновите Deploy Logs в тот же момент.
