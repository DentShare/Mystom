...
В Railway переменная `$PORT` в Custom Start Command может не подставляться. **Не используйте** команду вида `uvicorn ... --port $PORT`. Либо оставьте Start Command пустым и задайте **`SERVICE_TYPE=web`**, либо укажите явно: **пше** (скрипт сам читает PORT из окружения).
