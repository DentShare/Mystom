...
В Railway переменная `$PORT` в Custom Start Command может не подставляться. **Не используйте** команду вида `uvicorn ... --port $PORT`. Либо оставьте Start Command пустым и задайте **`SERVICE_TYPE=web`**, либо укажите явно: **`python -m admin_webapp.run_web`** (скрипт сам читает PORT из окружения).
